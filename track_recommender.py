...
def deterministic_track_recommendation(payload: Dict[str, Any], reason: Optional[str] = None) -> TrackRecommendation:
    ...
    if reason:
        notes = f"{notes} Azure error: {reason}"
...
def normalize_track_recommendation_payload(raw: Dict[str, Any], payload: Dict[str, Any]) -> Dict[str, Any]:
    """Best-effort normalization for slightly malformed LLM JSON."""
    normalized: Dict[str, Any] = dict(raw)

    raw_options = normalized.get("track_options")
        ...
        for item in raw_options:
            if isinstance(item, dict):
                rebuilt.append(
                    {
                        ...
                        "rationale": str(item.get("rationale", "")).strip() or "Recommended based on feasible overlap.",
                        "aligned_route_index": int(item.get("aligned_route_index", 1)),
                ...
                rebuilt.append(
                    {
                        "track": item.strip(),
                        "rationale": "Recommended based on feasible overlap.",
                        "aligned_route_index": 1,
                        "confidence": 0.5,
                    }
                )
        normalized["track_options"] = [x for x in rebuilt if x.get("track")]

    if "recommended_track" not in normalized or not str(normalized.get("recommended_track", "")).strip():
        opts = normalized.get("track_options", [])
        if isinstance(opts, list) and opts:
            first = opts[0]
            if isinstance(first, dict):
                normalized["recommended_track"] = str(first.get("track", "")).strip()

    if "notes" not in normalized:
        normalized["notes"] = "Generated by Azure LLM."
...
def recommend_tracks(
    ...
    system_prompt = (
        "You are a CS track advisor. Use ONLY tracks from payload.tracks and ONLY route indexes that exist in "
        "payload.feasible_routes. Return strict JSON only (no markdown, no prose) with this exact schema:\n"
        "{\n"
        '  "track_options": [\n'
        "    {\n"
        '      "track": "string from payload.tracks[*].track",\n'
        '      "rationale": "short reason",\n'
        '      "aligned_route_index": 1,\n'
        '      "confidence": 0.0\n'
        "    }\n"
        "  ],\n"
        '  "recommended_track": "one of track_options.track",\n'
        '  "notes": "single short sentence"\n'
        "}\n"
        "Rules: track_options must be a non-empty list of objects (not strings), confidence must be numeric "
        "between 0 and 1, aligned_route_index must be 1-based integer."
    )
    if not llm_client or not llm_client.available:
        return deterministic_track_recommendation(payload, reason="Azure LLM not configured")

    try:
        raw = llm_client.chat_json(system_prompt=system_prompt, user_payload=payload, max_tokens=1200)
        normalized = normalize_track_recommendation_payload(raw, payload)
        return parse_track_recommendation(normalized)
    except Exception as exc:
        return deterministic_track_recommendation(payload, reason=str(exc))
...
def generate_final_track_plan(
    ...
    system_prompt = (
        "You are a CS planning assistant. Produce a final plan for selected_track using ONLY courses from "
        "feasible_routes. Return strict JSON only with this exact schema:\n"
        "{\n"
        '  "final_plan_markdown": "string, bullet list by term",\n'
        '  "chosen_route_index": 1,\n'
        '  "courses_referenced": ["COMP_SCI 214", "COMP_SCI 308"],\n'
        '  "notes": "single short sentence"\n'
        "}\n"
        "Rules: chosen_route_index is a 1-based integer within feasible_routes length; "
        "courses_referenced must be a list of course codes from feasible_routes only."
    )
    if not llm_client or not llm_client.available:
        return deterministic_final_plan(selected_track, feasible_routes, route_hint, reason="Azure LLM not configured")
    ...
    try:
        ...
        if not validate_final_plan_courses(parsed.courses_referenced, feasible_routes):
            return deterministic_final_plan(
                selected_track,
                feasible_routes,
                parsed.chosen_route_index,
                reason="LLM referenced courses outside feasible routes",
            )
        return parsed
    except Exception as exc:
        return deterministic_final_plan(selected_track, feasible_routes, route_hint, reason=str(exc))
